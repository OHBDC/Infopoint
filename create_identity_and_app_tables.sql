...
IF NOT EXISTS (
    ...
    WHERE [MigrationId] = N'20251009095650_InitialSQLServerMigration'
)
BEGIN
...
BEGIN
    CREATE TABLE [Staff] (
        ...
        [Active] bit NULL,
        [Noupdate] bit NULL,
        [StaffReference] nvarchar(450) NOT NULL,
        CONSTRAINT [PK_Staff] PRIMARY KEY ([ID]),
        CONSTRAINT [AK_Staff_StaffReference] UNIQUE ([StaffReference])
    );
END;
GO
...
BEGIN
    CREATE TABLE [PDRs] (
        ...
        CONSTRAINT [FK_PDRs_Staff_StaffReference] FOREIGN KEY ([StaffReference]) REFERENCES [Staff] ([StaffReference]) ON DELETE CASCADE
    );
END;
GO
...
IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20251009102257_ExcludeStaffFromMigrations'
)
BEGIN
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20251009102257_ExcludeStaffFromMigrations', N'8.0.11');
END;
GO
